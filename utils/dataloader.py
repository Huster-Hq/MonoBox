    ...
    def __init__(self, image_root, trainsize):
        ...
        with open(image_root, 'r') as f:
            for pic in f.readlines():
                ...
                name = pic.split('/')[-1]
                if pic.find('.jpg')>-1:
                    gtbox_list.append(pic.replace('/images/'+name, '/noise_box_new/normal_0.2/ori_box_label/'+name.replace('.jpg', '.txt')))
                    true_gtbox_list.append(pic.replace('TrainDataset/images', 'TrainDataset/labels').replace('.jpg', '.txt'))
                else:
                    gtbox_list.append(pic.replace('/images/'+name, '/noise_box_new/normal_0.2/ori_box_label/'+name.replace('.png', '.txt')))
                    true_gtbox_list.append(pic.replace('TrainDataset/images', 'TrainDataset/labels').replace('.png', '.txt'))
        ...
        self.clean_box_file = [f for f in true_gtbox_list if f.endswith('.txt')]
        self.images = sorted(self.images)
        self.box_file = sorted(self.box_file)
        self.clean_box_file = sorted(self.clean_box_file)
        self.boxeslist = []
        self.imageslist = []
